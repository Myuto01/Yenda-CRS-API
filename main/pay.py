...
class PayClass():
    ...
    collections_subkey = os.getenv("collections_subkey")

    # Disbursement subscription key
    disbursements_subkey = os.getenv("disbursements_subkey")
    ...
    # Step 1: Define URLs and payloads
    ...
    if response.status_code != 201:
        print(f"Error: Failed to create API user. Status Code: {response.status_code}, Response: {response.text}")
    else:
        print("API user created successfully.")

    # Step 3: Create API key
    url = f"{accurl}/v1_0/apiuser/{disbursements_apiuser}/apikey"
    payload = {}
    headers = {
        'Ocp-Apim-Subscription-Key': disbursements_subkey
    }
    ...
    # Step 4: Print and debug the response
    print("API Key Response: ", response.text)

    # Step 5: Handle the response and extract apiKey
    try:
        response_data = response.json()
        api_key_disbursements = str(response_data["apiKey"])
    except KeyError:
        print("Error: 'apiKey' not found in the response.")
        api_key_disbursements = None
    except json.JSONDecodeError:
        print("Error: Failed to decode JSON response.")
        api_key_disbursements = None

    # Auto generate when in test mode
    if environment_mode == "sandbox" and api_key_disbursements:
        api_key_disbursements = str(response_data["apiKey"])

    # Create basic key for Collections
    if api_key_disbursements:
        ...
        print("Error: Unable to generate basic authorization string due to missing API key.")
